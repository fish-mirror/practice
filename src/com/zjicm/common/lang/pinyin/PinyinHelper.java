package com.zjicm.common.lang.pinyin;
...
public class PinyinHelper {
        ...
        for (int i = pinyinArray.length - 1; i >= 0; i--) {
            ...
            for (int j = originalPinyin.length() - 1; j >= 0; j--) {
                ...
                if (originalChar < 'a' || originalChar > 'z') {
                    ...
                    pinyinArray[i] =
                            originalPinyin.replaceAll(String.valueOf(originalChar), String.valueOf(replaceChar)) +
                            toneNumber;
                    hasMarkedChar = true;
                    break;
                }
     ...
     * @param pinyinFormat 拼音格式：WITH_TONE_NUMBER--数字代表声调，WITHOUT_TONE--不带声调，
     *                     WITH_TONE_MARK--带声调
     * @return 格式转换后的拼音
     */
    ...
    public static String convertToPinyinString(String str, String separator, PinyinFormat pinyinFormat) {
        ...
        for (int i = 0, len = str.length(); i < len; i++) {
            ...
            if (ChineseHelper.isChinese(c) || c == '〇') // 判断是否为汉字或者〇
            {
                ...
                for (int rightIndex = (i + rightMove) < len ? (i + rightMove) : (len - 1);
